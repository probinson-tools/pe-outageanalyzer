...
export default function ErrorTable({ errors }: Props) {
  ...
  return (
    <div className="glass rounded-2xl p-6">
      ...
      <p className="text-white/40 text-xs mb-4">{errors.length} distinct error types detected</p>
      <div className="space-y-2 max-h-96 overflow-y-auto scrollbar-thin pr-1">
        {errors.map((err, i) => (
          <div key={i} className="rounded-lg bg-white/3 border border-white/6 p-3 space-y-1.5">
            ...
            {err.sample && (
              <p className="text-white/30 text-xs font-mono leading-relaxed bg-white/3 rounded px-2 py-1.5 truncate">
                {err.sample}
              </p>
            )}
          </div>

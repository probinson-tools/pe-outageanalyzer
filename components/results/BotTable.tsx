...
export default function BotTable({ bots }: Props) {
  return (
    <div className="glass rounded-2xl p-6">
      <h3 className="text-slate-100 font-semibold mb-1">Suspicious Traffic &amp; Bots</h3>
            ...
            <div key={i} className="rounded-lg bg-white/3 border border-white/8 p-3 space-y-1.5">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-200 text-sm font-mono">{bot.ip}</span>
                </div>
              ...
              <p className="text-slate-600 text-xs font-mono truncate">{bot.userAgent}</p>
            </div>
          ))}
        </div>
      )}

    ...
    <div className="fixed inset-0 z-50 bg-[#0f1117]/90 backdrop-blur-sm flex items-center justify-center">
      <div className="glass rounded-2xl p-10 max-w-sm w-full mx-4 text-center space-y-6">
        <div className="relative w-16 h-16 mx-auto">
          ...
          <div className="absolute inset-2 rounded-full border-2 border-t-red-400 border-white/10 animate-spin"></div>
        ...
        <div>
          <p className="text-white font-semibold">Claude is analyzing your logs</p>
          <p className="text-white/40 text-sm mt-1 h-5 transition-all">{STEPS[step]}</p>
        </div>
        <div className="flex gap-1 justify-center">
          {STEPS.map((_, i) => (
            <div key={i} className={`h-1 rounded-full transition-all duration-500 ${i === step ? "w-6 bg-red-400" : "w-1.5 bg-white/15"}`} />
          ))}
        </div>
      </div>
    </div>
